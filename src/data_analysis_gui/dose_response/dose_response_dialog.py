...
from PySide6.QtWidgets import (
    ...
    QSplitter, QGroupBox, QTableWidget,
...
from data_analysis_gui.dose_response.concentration_range_table import ConcentrationRangeTable
from data_analysis_gui.dose_response.cursor_pyqtgraph import PyQtGraphCursorManager
from data_analysis_gui.dose_response.interactive_range_creator import InteractiveRangeCreator
from data_analysis_gui.dose_response.dose_response_service import ConcentrationResponseService
from data_analysis_gui.dose_response.dose_response_models import AnalysisRange
from data_analysis_gui.dose_response.dose_response_exporter import ConcentrationResponseExporter
...
class ConcentrationResponseDialog(QDialog):
    ...
    def _create_ranges_group(self) -> CollapsibleGroupBox:
        ...
        # Tab 2: Calculator
        from data_analysis_gui.dose_response.range_calculator_widget import RangeCalculatorWidget
    ...
    def _create_plot_panel(self) -> QGroupBox:
        ...
        # Coordinate display label (top-right of plot area)
        self.coord_label = pg.TextItem(
            text="",
            anchor=(1, 0),  # anchor at label's top-right corner
            color='#2D3436',
            fill=(255, 255, 255, 200),
            border=pg.mkPen(color='#B0B0B0', width=0.5)
        )
        self.coord_label.setZValue(1000)  # render above plot contents
        self.plot_widget.scene().addItem(self.coord_label)
        
        # Keep label positioned on resize, update text on mouse move
        self.plot.getViewBox().sigResized.connect(self._update_coord_label_position)
        self.plot_widget.scene().sigMouseMoved.connect(self._on_mouse_moved_over_plot)
    ...
    def _update_coord_label_position(self):
        """Anchor coordinate label to top-right of the plot's viewbox."""
        vb = self.plot.getViewBox()
        geom = vb.sceneBoundingRect()
        # 8px right margin, 4px top margin
        self.coord_label.setPos(geom.right() - 8, geom.top() + 4)
    
    def _on_mouse_moved_over_plot(self, scene_pos):
        """Update coordinate label with mouse position in data coordinates."""
        vb = self.plot.getViewBox()
        ...
        self.coord_label.setText(
            f"x: {view_pos.x():.3f}, y: {view_pos.y():.3f}"
        )

...
# Needs larger tolerance of voltage differences defined as the same voltage point 
# Generalized summary export better for now    

# class GVAnalysisService:
#     """
#     Provides services for preparing and analyzing G-V (Conductance-Voltage) data.

#     This service transforms raw batch results into formats suitable for G-V curve analysis,
#     supporting both single and dual-range data.
#     """
...
#     ) -> Tuple[Dict[float, list], Dict[str, str], Optional[Dict[float, list]]]:
#         """
#         Transform raw batch results into a format suitable for G-V curve analysis.
...
#         Returns:
#             Tuple[Dict[float, list], Dict[str, str], Optional[Dict[float, list]]]:
#                 - Dictionary mapping rounded voltages to lists of conductance values (range 1).
#                 - Dictionary mapping recording IDs to base filenames.
#                 - Dictionary mapping rounded voltages to lists of conductance values (range 2), or None if not used.
#         """
#         logger.info(f"Preparing GV data from {len(batch_results)} batch results")
#         logger.debug(f"Dual-range analysis: {params.use_dual_range}")
        
#         gv_data_range1: Dict[float, list] = {}
#         gv_data_range2: Optional[Dict[float, list]] = (
#             {} if params.use_dual_range else None
#         )
#         gv_file_mapping: Dict[str, str] = {}

#         # Condition check for GV analysis
#         is_gv_analysis = (
#             params.x_axis.measure in ["Average", "Peak"]
#             and params.x_axis.channel == "Voltage"
...
#         if not is_gv_analysis:
#             logger.warning(
#                 f"Not GV analysis configuration - X: {params.x_axis.measure} {params.x_axis.channel}, "
#                 f"Y: {params.y_axis.measure}"
#             )
#             return gv_data_range1, gv_file_mapping, gv_data_range2

#         logger.debug(f"Valid GV analysis configuration detected")
...
#                 for x_val, y_val in zip(data["x_values"], data["y_values"]):
#                     rounded_voltage = round(x_val, 1)
#                     if rounded_voltage not in gv_data_range1:
#                         gv_data_range1[rounded_voltage] = []
#                     gv_data_range1[rounded_voltage].append(y_val)
#             else:
#                 logger.warning(f"  Missing x_values or y_values for {base_name}")
...
#                     for x_val2, y_val2 in zip(data["x_values2"], data["y_values2"]):
#                         rounded_voltage = round(x_val2, 1)
#                         if rounded_voltage not in gv_data_range2:
#                             gv_data_range2[rounded_voltage] = []
#                         gv_data_range2[rounded_voltage].append(y_val2)
#                 elif "y_values2" in data:
#                     logger.warning(
#                         f"  Range 2 missing separate x_values for {base_name}, using Range 1 x_values"
#                     )
...
#             recording_id = f"Recording {idx + 1}"
#             gv_file_mapping[recording_id] = base_name

#         # Log summary statistics
#         num_voltages_r1 = len(gv_data_range1)
#         logger.info(f"Range 1: Collected {num_voltages_r1} unique voltage points")
#         if gv_data_range2 is not None:
#             num_voltages_r2 = len(gv_data_range2)
#             logger.info(f"Range 2: Collected {num_voltages_r2} unique voltage points")
        
#         logger.debug(f"File mapping created with {len(gv_file_mapping)} entries")

#         return gv_data_range1, gv_file_mapping, gv_data_range2


# class GVSummaryExporter:
#     """
#     Handles exporting GV summary data without conductance density calculations.
...
#     @staticmethod
#     def prepare_summary_table(
#         gv_data: Dict[float, list],
#         gv_file_mapping: Dict[str, str],
#         included_files: set = None,
#         conductance_units: str = "nS",
#     ) -> Dict[str, Any]:
#         """
#         Prepare GV summary data for export with unit-aware headers.

#         Organizes GV data for CSV export, including voltage and conductance columns for each recording.
#         Handles missing data by inserting NaN values.

#         Args:
#             gv_data (Dict[float, list]): Dictionary mapping voltages (float) to lists of conductance values.
#             gv_file_mapping (Dict[str, str]): Dictionary mapping recording IDs to file names.
#             included_files (set, optional): Set of file names to include (None = all files).
#             conductance_units (str, optional): Units for conductance measurements ('S', 'mS', 'μS', 'nS', 'pS'). Defaults to 'nS'.
...
#         logger.info(f"Preparing GV summary table with {len(gv_data)} voltage points")
#         logger.debug(f"Conductance units: {conductance_units}")
...
#         # Get sorted voltages
#         voltages = sorted(gv_data.keys())
        
#         if not voltages:
#             logger.error("No voltage data available for GV summary")
...
#         # Sort recordings
#         sorted_recordings = sorted(
#             gv_file_mapping.keys(), key=lambda x: int(x.split()[-1])
#         )
#         logger.debug(f"Processing {len(sorted_recordings)} recordings")
...
#         for recording_id in sorted_recordings:
#             base_name = gv_file_mapping.get(recording_id, recording_id)
...
#             # Add header with conductance units
#             headers.append(f"{base_name} ({conductance_units})")
#             recording_index = int(recording_id.split()[-1]) - 1
...
#             nan_count = 0
#             for voltage in voltages:
#                 if recording_index < len(gv_data[voltage]):
#                     conductance_values.append(gv_data[voltage][recording_index])
#                 else:
...
#             data_columns.append(conductance_values)

...
Parsing logic inspired by pyABF, modified for ABF1 imports to match WinWCP results.
For some reason, pyABF's ABF1 parsing (at least for ABF files exported by WinWCP) 
yields different time index results than WinWCP. There is a discrepancy of a factor of 2 in the time values. 
So a sweep at 250s will be read as 125s by pyABF. It is unclear if this is a bug in the WinWCP export or pyABF's parsing.

This script may be updated in the future to only use pyABF if I can determine the source of the discrepancy. If WinWCP
has the error, then this script may erroneously double the time values of each sweep. More data needed to make a determination.
"""

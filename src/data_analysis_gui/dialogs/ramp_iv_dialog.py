...
class VoltageInputDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        # saved_voltages = settings.get('ramp_iv_voltages', "-80, -60, -40, -20, 0, 20, 40, 60, 80") if settings else "-80, -60, -40, -20, 0, 20, 40, 60, 80"
        self.voltage_input.setPlainText(_load_ramp_iv_voltages())

        layout.addWidget(self.voltage_input)
        
        # Validation note
        # note = QLabel("Note: Values should be within ±200 mV range.")
...
class RampIVDialog(QDialog):
    ...
    def __init__(self, dataset: ElectrophysiologyDataset, 
        ...
        # # Get voltage targets first via input dialog
        # if not self._get_voltage_targets():
        #     self.reject()
        #     return

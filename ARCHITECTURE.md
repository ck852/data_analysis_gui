...
MainWindow (main_window.py) is the entry point for user interaction. It contains the ControlPanel (widgets/control_panel.py) where users set analysis parameters (time ranges, measure types, peak modes) and PlotManager (plot_manager.py) which displays raw sweep data from the user's selected file. When the user clicks "Generate Analysis Plot" or triggers a batch analysis, MainWindow assembles an `AnalysisParameters` (core/params.py) object from the parameters set in the present UI state and passes it to the ApplicationController (core/app_controller.py).

### Layer 2: ApplicationController

ApplicationController is the coordination layer between GUI and the backend. It maintains the application state; specifically `current_dataset` (the loaded file's data) and `loaded_file_path`. When MainWindow requests an analysis, ApplicationController validates that data is loaded, then forwards the request to AnalysisManager (services/analysis_manager.py). This separation of file loading from MainWindow facilitates the program's flexibility to perform repeated analyses on a set of files.

ApplicationController also holds references to the three main services: DataManager (services/data_manager.py) for file I/O, AnalysisManager for analysis orchestration through the deeper service layers, and BatchProcessor (services/batch_processor.py) for multi-file operations. It routes data analysis requests to the appropriate service and wraps responses in result objects that include success/failure status and error messages. For batch operations, ApplicationController delegates to BatchProcessor, which internally creates its own DataManager instances to process each file in isolation.
...
When `analyze()` is called, AnalysisManager passes the dataset and parameters to AnalysisEngine, receives back a list of `SweepMetrics`, and wraps them in an `AnalysisResult` for return to the controller. It also provides convenience methods like `get_export_table()` and `get_sweep_plot_data()` that call through to the engine with appropriate formatting to export plot-ready data for visualization in external applications.

### Layer 4: AnalysisEngine

AnalysisEngine is the orchestrator of the actual computation. It joins three components: DataExtractor (core/data_extractor.py), which gets raw data arrays from the currently loaded dataset, MetricsCalculator (core/metrics_calculator.py), which performs all numerical processing operations on this data, and PlotFormatter (core/plot_formatter.py), which transforms metrics into plot-ready and/or export-ready data structures. 
...
If dual range is enabled, the same calculations run on range2, populating the `_r2` fields of SweepMetrics. All possible core analyses (average and all peak modes for both channels) are performed by MetricsCalculator; PlotFormatter controls which analyses are retained for the user. 

### Layer 7: PlotFormatter

PlotFormatter transforms raw `SweepMetrics` lists into structures suitable for plotting or export. This is where user selections (Average vs Peak, Voltage vs Current, which peak type) get applied.

When `format_for_plot()` is called, PlotFormatter reads the `AxisConfig` from parameters to determine which axes to plot which data on. If Y-axis is "Average Current", it extracts `current_mean_r1` from each SweepMetrics. If it's "Peak Current" with peak_type "Negative", it extracts `current_negative_r1`. The formatter builds parallel arrays of x and y values for plotting.
...
User clicks "Generate Analysis Plot"
    ↓
ControlPanel.get_parameters() → AnalysisParameters
    ↓
ApplicationController.perform_analysis(params, rejected_sweeps)
    ↓
AnalysisManager.analyze(dataset, params, rejected_sweeps)
    ↓
AnalysisEngine.get_plot_data(dataset, params, rejected_sweeps)
    ↓
AnalysisEngine.analyze_dataset(dataset, params, rejected_sweeps)
    ↓
AnalysisEngine._compute_all_metrics(dataset, params, rejected_sweeps)
    ↓ (for each sweep)
...
PlotFormatter.format_for_plot(metrics, params) → {x_data, y_data, labels}
    ↓
Returns up through each layer as AnalysisResult
    ↓
